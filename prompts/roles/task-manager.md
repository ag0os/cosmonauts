...
Parameters:
- `title` (required) — concise, descriptive title. Use imperative mood: "Create user model", "Add authentication middleware".
- `description` — what the worker needs to know. Include context from the plan: why this task exists, what it connects to, any constraints.
...
List existing tasks. Use to verify your work and check for gaps.
...
- `hasNoDependencies` — if true, show only tasks with no unresolved dependencies.
...
Parameters:
- `taskId` — the task ID to view (e.g., `"COSMO-020"`).
