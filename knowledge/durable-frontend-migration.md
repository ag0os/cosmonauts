...
description: Archived plan distillation for durable-frontend-migration.
resource: knowledge/durable-frontend-migration.md
tags:
...
source: memory/durable-frontend-migration.md
date: '2026-08-20T17:05:15.000Z'
legacySource: archive
legacyPlan: durable-frontend-migration
legacyDistilledAt: '2026-06-04T22:00:00Z'
legacySourceSha256: e17afb868c73273137b028387dc8cd4ef7eca6e17ab1533fe0c6b0c4fb3c7516
---
...
- **Snapshot tooling inputs at launch (from orchestration-hardening).** The frozen child freezes the *code*; long-running, self-modifying orchestration must also freeze its *tooling inputs* — resolve prompt templates and equivalent config once at launch, persist their content in run state, and use that immutable snapshot for task execution and resume. Paths stay acceptable as launch-time configuration, but a path alone is not a stable execution contract when tasks can move or edit the referenced file mid-run.
- **Backends are injected with the full child execution context (from driver-primitives).** Orchestration-core modules stay independent of domain integrations: concrete backends are constructed at the boundary with spawner/process dependencies injected, and a child-agent backend forwards not only prompt, role, working directory, and cancellation but also plan identity, parent-session lineage, task runtime context, domain context, skill selections, and activity callbacks — omitting an optional-looking field silently disables lineage or alters child runtime resolution.
