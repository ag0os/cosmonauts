...
description: Archived plan distillation for domain-config.
resource: knowledge/domain-config.md
tags:
  - 'plan:domain-config'
...
source: memory/domain-config.md
date: '2026-08-20T17:05:15.000Z'
legacySource: archive
legacyPlan: domain-config
legacyDistilledAt: '2026-04-17T00:00:00.000Z'
legacySourceSha256: 7e71d9a625522bf7b171f9c3a4ea6776b1403a776b216734a53bbcb0ea0519f2
---
...
- **Resource resolution is three-tier, uniform across kinds (from package-system):** the requesting agent's domain, then every portable domain in discovery order, then the shared baseline — first match wins — applied identically to capabilities, extensions, reusable personas, and skill discovery, so overrides stay predictable and consumers cannot invent divergent fallback behavior.
- **Default-domain semantics live in one framework-owned resolver (from coding-agnostic-framework):** an explicit domain passes through unchanged; with a loaded registry, the resolver verifies the default domain is actually installed and otherwise raises an actionable error requiring an explicit domain; without a registry it may return the conventional default and let the file-specific lookup report what's missing. The same rule covers framework-owned orchestration defaults: default prompt substrate lives under a framework-owned location — not inside an optional domain — and every entrypoint that omits an override (CLI, agent tool) calls the same resolver. A source-scan test keeps fallback literals from reappearing at call sites.
- **CLI runnability is one pure predicate; an empty store is a valid first-run state (from coding-agnostic-framework + framework-extraction; supersedes the infrastructure-domain-guard gotcha in knowledge/main-domain-and-cosmo-rename/).** A runtime containing the shared substrate plus the domain that owns the default assistant is runnable; a shared-only or empty runtime is not — and that single predicate is reused by interactive mode, print mode, initialization, and the no-domain guard so modes cannot disagree. The absence of functional domains is a supported state, not corruption: package install/remove/list/create/update must initialize without an agent domain, and agent modes stop before dispatch with concrete installation choices — the only recovery path from an empty installation. Guard messages stay domain-neutral and actionable.
- **Domain prompt directories contain personas only (from domain-authoring; supersedes shared-owns-base modelling).** Each domain's prompt directory holds persona files named in parallel with agent definitions; universal base prompts and runtime overlays load from explicit framework-owned locations with an injectable framework prompt root for tests and embeddings. Domain content stays intrinsic and portable, a shared fallback domain never owns framework behavior, and a missing-persona diagnostic can name one predictable expected path.
- **Duplicate domain-ID reconciliation:** the merge-by-default conflict handling (union with later-source-wins) applies to duplicates at *unequal* precedence; equal-precedence duplicates are a hard failure with per-provider provenance in the report. (Reconciles the two promoted package-system and domain-authoring records, which each state one half.)
- **Framework tests use minimal synthetic installable domains (from coding-agnostic-framework):** a small function-based fixture writes a real package manifest, domain definition, agents, prompts, capabilities, and skills, loaded through production package discovery. Generic fixtures stay domain-neutral; a synthetic package uses a production domain id only when the behavior explicitly concerns that user-facing id.
