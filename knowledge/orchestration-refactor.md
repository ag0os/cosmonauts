...
description: Archived plan distillation for orchestration-refactor.
resource: knowledge/orchestration-refactor.md
tags:
...
source: memory/orchestration-refactor.md
date: '2026-08-20T17:05:15.000Z'
legacySource: archive
legacyPlan: orchestration-refactor
legacyDistilledAt: '2026-04-17T00:00:00.000Z'
legacySourceSha256: b0566128087cb942e77b590cde2d586ff66aa723a835754d5be5880e7d427bc0
---
...
- **Lock observable behavior before complexity refactors (from fallow-temp-exceptions-cleanup).** Before removing a complexity suppression, characterize the function's observable responsibilities with tests — adding missing failure/edge cases first — then re-run the same focused tests after restructuring plus the full suite. Private functions and command callbacks need no test-only exports when reachable through an existing public boundary.
- **Match the decomposition pattern to the source of complexity (from fallow-temp-exceptions-cleanup):** phase helpers for lifecycle flows, strategy helpers for priority cascades, rule helpers for validation, predicate composition for filters, section renderers for reports, per-variant dispatch for closed event families. Keep orchestration wrappers short with public contracts preserved; plain extraction is the fallback when no stronger behavioral boundary exists — and relocating the same branching wholesale into an unrelated helper reduces nothing.
- **Deduplicate tests without hiding their intent (from fallow-temp-exceptions-cleanup).** Share a test helper only when setup repeats across three or more call sites or files *and* represents a stable concept; otherwise prefer a file-local builder, a table for symmetric cases, or a small inline collapse — keeping assertion-specific values visible in each test. Some local repetition is the accepted price of diagnostic failures and one-concept-per-test readability.
