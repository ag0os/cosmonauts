...
describe("createSkillsProgram export — compatibility facade", () => {
	it("exports a named skill through the shared provenance engine", async () => {
		const { mkdir, mkdtemp, readFile, rm, writeFile } = await import(
			"node:fs/promises"
		);
		const { tmpdir } = await import("node:os");
		const { join } = await import("node:path");
		const root = await mkdtemp(join(tmpdir(), "skills-export-cli-"));
		const originalCwd = process.cwd();
		const log = vi.spyOn(console, "log").mockImplementation(() => {});
		try {
			const domainRoot = join(root, "alpha");
			const skillDir = join(domainRoot, "skills", "plan");
			...
			process.chdir(root);
			runtimeMocks.create.mockResolvedValue({
				domains: [makeDomain("alpha", domainRoot)],
				projectConfig: {},
			});
			runtimeMocks.discoverFrameworkBundledPackageDirs.mockResolvedValue([]);

			await createSkillsProgram().parseAsync(
				["export", "--target", "claude", "plan"],
				{ from: "user" },
			);

			const target = join(root, ".claude", "skills", "plan", "SKILL.md");
			expect(await readFile(target, "utf8")).toContain(
				"Generated by cosmonauts",
			);
			expect(
				JSON.parse(
					await readFile(
						join(root, ".claude", ".cosmonauts-harness-manifest.json"),
						"utf8",
					),
				),
			).toMatchObject({ schemaVersion: 1 });
			expect(log).toHaveBeenCalledWith(
				expect.stringContaining("skill:alpha/plan"),
			);

			log.mockClear();
			await createSkillsProgram().parseAsync(
				["export", "--target", "claude", "--all"],
				{ from: "user" },
			);
			expect(log).toHaveBeenCalledWith(
				expect.stringContaining("skill:alpha/plan"),
