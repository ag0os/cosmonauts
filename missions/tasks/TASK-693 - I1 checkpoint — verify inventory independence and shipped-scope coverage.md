...
updatedAt: '2026-09-17T16:44:44.680Z'
---
...
- [ ] #4 The current epoch’s inventory and source-log digests are frozen, internally consistent, and pass schema plus B-006 artifact-conformance checks. Inventory freshness is scoped as in TASK-692 AC #6 — a later **non-test authority or inventory** change requires a successor epoch — and per D-035 this gate does **not** require the epoch's `evaluatedRevision` to equal HEAD; revision-pinning binds at the TASK-703 candidate epoch certified by E1.
<!-- AC:END -->

## Implementation Notes

Run-6 attempt (2026-09-17) passed AC #1-3 and failed AC #4 by requiring the epoch's
`evaluatedRevision` to equal HEAD. That check was correct against the AC as written and
wrong against ratified ground: `spec.md:291` pins revision-specificity to the baseline,
and `AC-016` forbids a stage that needs human help to proceed. See D-035.
