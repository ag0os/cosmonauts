...
- [ ] #5 Retirement authority revalidates exact promotion or human `ratifiedBaselines` bytes, complete receipts/citations, current digest, cap, lock, and manifest-state guards under the lock; unknown or later-curated baselines, inbound/incomplete inventories, changed bytes, active-retired live paths, and unchanged restored paths remain live with explicit conflicts.
