...
updatedAt: '2026-07-30T16:29:50.582Z'
---
...
<!-- AC:BEGIN -->
- [ ] #1 `B-031` — `tests/prompts/quality-manager.test.ts` > `preserves explicit migration reference searches even when dead code is bound` proves both `bundled/coding/prompts/worker.md` and `bundled/coding/prompts/quality-manager.md` instruct running the dead-code capability when bound AND always running the explicit old-identifier/path search across runtime source, tests, and docs.
- [ ] #2 The test proves the explicit search is unconditional — a bound dead-code capability does not make it optional — and both prompts state why: structural reachability cannot prove stale strings absent.
- [ ] #3 The worker's existing runtime-source-first sweep ordering and its rule that a tests/docs-only sweep is not sufficient survive unchanged.
- [ ] #4 Neither prompt names a provider or a command for the capability half (`INV-1`).
- [ ] #5 The TASK-537 Quality Manager migration-sweep characterization still passes, the test carries `@cosmo-behavior plan:analysis-gate-rewiring#B-031` near the executable test, and the project's test, lint, and type-check steps pass.
<!-- AC:END -->
