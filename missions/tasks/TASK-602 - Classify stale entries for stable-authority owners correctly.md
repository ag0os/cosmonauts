...
<!-- AC:BEGIN -->
- [ ] #1 A stale manifest entry owned by a matching stable authority classifies as the same owner, not `foreign-owner`.
- [ ] #2 An authority-owned asset whose source is absent from a healthy still-declared root reports `source-removed` and gets D-016 safe exits (unchanged target plus entry removed; absent target entry removed; edited target preserved pending explicit `--forget-removed`).
- [ ] #3 Genuinely foreign owners — a different project owner, or a different authority id — still classify as `foreign-owner` and are never implicit removal candidates.
- [ ] #4 Incomplete or undeclared source health still blocks any removal for authority-owned entries, exactly as for project-owned ones.
- [ ] #5 Existing B-004/B-006 tests pass under their existing titles and markers; no new behavior marker is added.
<!-- AC:END -->
