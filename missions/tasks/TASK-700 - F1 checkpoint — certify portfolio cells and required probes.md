...
<!-- AC:BEGIN -->
- [ ] #1 No critical `protected` portfolio is missing an applicable boundary/path/caller/defect-axis cell or a probe required by any of the five unconditional triggers.
- [ ] #2 Every executed required probe satisfies copied-graph containment, isolated-defect, expected-red or survived outcome, source-checkout preservation, and restored-sandbox-green evidence; `git checkout` appears in no restoration path.
...
- [ ] #4 B-007 and B-008 correctness and exact marker artifact-conformance pass, while the generic `mutation` gate is explicitly recorded as unbound, not enforced, and subject to the plan’s targeted evidence/reviewer judgment.
<!-- AC:END -->
