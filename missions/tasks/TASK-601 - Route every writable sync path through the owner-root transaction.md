...
updatedAt: '2026-08-26T13:48:15.778Z'
---
...
<!-- AC:BEGIN -->
- [x] #1 No exported function can write a target or manifest outside an owner-root transaction; either the write path is routed through `withOwnerRootTransaction`/`applySyncPlanInTransaction`, or the writable entry point is made non-public and callers migrated.
- [x] #2 A target edited concurrently between classification and commit is preserved, not erased, on every writable path.
- [x] #3 Target and manifest can never be left inconsistent by a crash between their writes on any writable path.
- [x] #4 A test proves the previously-unlocked path now honors the lock and journal, and that a concurrent edit survives it.
- [x] #5 The production `harness sync` and compatibility `skills export` behavior is unchanged; existing B-005/B-007 tests pass under their existing titles and markers.
<!-- AC:END -->
