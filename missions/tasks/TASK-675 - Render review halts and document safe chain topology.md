...
updatedAt: '2026-09-10T02:15:16.402Z'
---
...
- [ ] #2 B-012 is proven by `tests/extensions/orchestration-rendering.test.ts` > `renders an unaddressed review-round halt in chain progress`, carrying `@cosmo-behavior plan:chain-stage-context#B-012`: progress says task decomposition was halted, includes available identity/reason, and remains defined when identity is absent.
- [ ] #3 Both renderers consume the same typed `unaddressed_review_round` event contract produced by inline and durable paths without inventing renderer-local review state.
- [ ] #4 `docs/orchestration.md` documents topology-derived revision purpose, the exact reviewer and revision terminal-report responsibilities, the strictly-earlier addressed-evidence task guard, safe sequential versus unsafe same-index topology, and correction-plus-rerun guidance.
- [ ] #5 The existing CLI command/option surface, planner persona, external `/spec-to-backlog` flow, and unrelated output strings remain unchanged.
<!-- AC:END -->
