...
updatedAt: '2026-07-31T15:45:17.724Z'
---
...
<!-- AC:BEGIN -->
- [ ] #1 Normalization raises an `AnalysisFailure` when any normalized finding carries a category outside the result's declared coverage, on both single-capability and changed-scope-audit paths
- [ ] #2 The contradiction is never reconciled by widening the declared coverage or by dropping the offending finding
- [ ] #3 The raised failure preserves the provider evidence and names capability, provider, failure class, and process evidence in the shape established by D-020
- [ ] #4 A `boundary-conformance` finding produced by a dead-code or audit invocation with boundary zones and rules configured still normalizes to a result rather than a failure
- [ ] #5 `tests/extensions/project-tools-fallow.test.ts` proves both a boundary finding with no configured zones and a non-boundary cross-category case are rejected, in a test named `rejects findings outside the declared gate coverage` carrying marker `@cosmo-behavior plan:analysis-gate-coverage#B-042`
