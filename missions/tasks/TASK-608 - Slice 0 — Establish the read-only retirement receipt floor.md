...
updatedAt: '2026-09-01T21:48:40.689Z'
---
...
<!-- AC:BEGIN -->
- [x] #1 B-001 is green in `tests/memory/interface.test.ts` > `accepts only manifest-backed relocation and ledger-backed hard deletion`, carrying exact marker `@cosmo-behavior plan:living-memory#B-001` near the executable test and proving exact-byte manifest relocation, ledger-only hard deletion, and fail-closed serialization/path/digest/body/history mutations.
- [x] #2 The receipt inventory folds healthy promotion ledgers including `retiredRecords` and `ratifiedBaselines` plus ordered retirement/restoration manifests, while malformed, duplicate, noncontiguous, unknown, unsafe, or unreadable state remains unhealthy and cannot authorize absence.
- [x] #3 Slice 0 contains no function capable of linking, renaming, unlinking, deleting, or otherwise relocating a path under `knowledge/`, and the existing frozen metadata/body/destination/index protections remain intact.
- [x] #4 Project-native universal correctness evidence passes after every commit; once B-001 lands it remains green for every commit in this slice and all later slices, with no relocation-capable commit permitted while its receipt evidence is red.
<!-- AC:END -->
