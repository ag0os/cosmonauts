...
updatedAt: '2026-06-04T14:17:34.367Z'
---
...
<!-- AC:BEGIN -->
- [ ] #1 B-007 is implemented and proven by `tests/durable-runtime/scheduler-retry.test.ts` > `retries with a new attempt record and preserves prior failed attempt evidence`, including marker `@cosmo-behavior plan:durable-graph-scheduler#B-007`.
- [ ] #2 B-008 is implemented and proven by `tests/durable-runtime/scheduler-retry.test.ts` > `blocks unknown results and exhausted retries instead of advancing dependents`, including marker `@cosmo-behavior plan:durable-graph-scheduler#B-008`.
- [ ] #3 Retry transitions append new attempts, preserve previous evidence, never retry completed steps, and block malformed/unknown/exhausted outcomes without advancing dependents.
