...
updatedAt: '2026-06-30T17:36:42.876Z'
---
...
<!-- AC:BEGIN -->
- [ ] #1 B-003: create allocates above an archived-only filename maximum even when the archived file body is missing/malformed (no archived ID reuse, no content parsing required)
- [ ] #2 B-004: a missing/empty archive directory is treated as empty and creation still succeeds (empty project -> first configured ID)
- [ ] #3 B-005: create leaves an existing config.json byte-unchanged and ignores any legacy lastIdNumber
- [ ] #4 B-006: creating the first task when no config exists does not create config.json
- [ ] #5 B-007: archived tasks influence allocation only — list/search/get/update/delete stay active-only and never return or edit archived files
- [ ] #6 B-009: init() returns and caches config without lastIdNumber, and all allocation reads happen inside the create lock
<!-- AC:END -->
