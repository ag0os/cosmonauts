...
- [ ] #2 B-004 is GREEN: retrieval filters physical scope first, reflects edits/retimes/deletions from current disk, matches normalized metadata and body text, orders newest-first by explicit canonical timestamp or mtime fallback with path tie-break, and applies limit only after filtering.
