...
- [ ] #2 Every raw error, mismatch, unsupported construct, phase-unknown event, skip/todo/filter, conditional assertion candidate, flaky result, and assessment limitation has a visible state and disposition path.
- [ ] #3 C2 cannot pass while any required run is missing or while the census is `incomplete` or `blocked`; a post-run policy exit is classified only under the D-025 evidence rule.
