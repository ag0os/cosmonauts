...
Perform only the mandatory fresh review in Implementation Order step 12; behavior ownership is none. Review the complete Stage-2 permitted implementation diff in a fresh context. Tell the reviewer that Stage 1 replaced lossy reconstruction with exact render input and that the prior round-7 fix still treated warned omissions as absence. Record the review under D-010; its artifact/task evidence does not authorize implementation outside the bounded stage.

Ratified-ground handling: the common constraints and fresh-review threshold are stop-and-escalate ground. A high/medium finding prevents completion and stage handoff; remediation must remain in Stage 2, begin with a counterexample, and receive another fresh review.
...
- [x] #4 (Quality Contract assertion 7; Implementation Order steps 11-12) Gates: `bun run test`, `bun run lint`, `bun run typecheck` and `git diff --check` all pass at the task's commit boundary. No test is committed red.
...
- [x] #6 (Quality Contract assertions 3 and 8; Implementation Order step 12) A fresh reviewer, explicitly given the Stage-1 and round-7 history, inspects healthy, byte-deferred-but-inventoried, warned-corpus-omission, malformed-episode, and contract-invalid-custom-source outcomes and verifies every completeness barrier seam: discharge, represented-evidence/no-work conclusion, judgment/proposal materialization, retirement authorization, warning/pressure reporting, and preservation of earlier recovery details; the Stage-1 permanent measurement pack and parent B-016/B-021 regressions are rerun.
- [ ] #7 (Quality Contract assertion 8; Implementation Order step 12) The fresh round's verdict is recorded in `missions/plans/living-memory-fidelity/review-<n>.md` and referenced in task evidence, and Stage 2 closes only with zero unresolved high or medium findings; any such finding starts a new bounded Stage-2 RED/GREEN/refactor remediation and another fresh review before Stage 3 may begin.
