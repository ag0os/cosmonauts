...
updatedAt: '2026-09-08T20:11:37.183Z'
---
...
- [ ] #8 (CDX-001; understating direction preserved) A `markMaterialized()` call that DOES write still reports `writesCommitted: true`, including when it throws after the bytes changed — the committed-error tagging from earlier rounds is unaffected. Both directions are pinned by tests on the same code path.
- [ ] #9 (CDX-002) `ConsolidationSourceFinalization` is exported from the source-contract block in `lib/memory/index.ts` alongside its siblings, and a test asserts it is nameable from the public barrel.
- [ ] #10 (regression) SR-010 path parity, the SR-011..SR-014 cause separation, the Stage-2 completeness barrier and the Stage-3/4 committed-write recording are preserved. The full fidelity pack B-001..B-012, the commit-interleaving tests and the exact parent B-012/B-016/B-021 carrier tests all pass.
<!-- AC:END -->
