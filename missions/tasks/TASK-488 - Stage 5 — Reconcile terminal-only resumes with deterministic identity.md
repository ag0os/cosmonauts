...
updatedAt: '2026-07-22T15:52:15.571Z'
---
...
<!-- AC:BEGIN -->
- [ ] #1 B-011 is proven by `tests/cli/drive/graph-resume.test.ts` > `records one run-id-derived terminal for an off-then-enabled completed resume`: source and deterministic run-id attempt identity are persisted before the early return, existing completedAt is preserved or stamped once, and exactly one terminal but no start episode is recorded, with the exact B-011 marker near the test.
- [ ] #2 B-012 is proven by `tests/cli/drive/graph-resume.test.ts` > `repeats deterministic terminal-only resume without changing bytes or episode count`: a fresh repeat derives the identical id and leaves spec, completion, ledger count, and episode count byte-idempotent, with the exact B-012 marker near the test.
- [ ] #3 B-013 is proven by `tests/memory/interface.test.ts` > `documents deterministic off-then-enabled terminal-only resume`: `docs/memory.md` adds the specified deterministic success/dedupe and honest warn/skip contract, qualifies the existing terminal-evidence claim, and preserves the launchDetached hard-kill and launch-resolution sentences, with the exact B-013 marker near the test.
...
- [ ] #5 The pre-`prepareResume()` helper runs only when `remainingTaskIds` is empty and no graph resume state exists; refused dirty-worktree or unsupported-backend resumes leave `spec.json` and `task-queue.txt` byte-identical.
- [ ] #6 `lib/driver/episode-identity.ts` uses a namespaced SHA-256 of persisted run id for the synthetic `attempt-...` token, contains no random source for F-005, and leaves the ordinary new-attempt mint unchanged; gate-OFF resume creates no runtime or new artifact.
- [ ] #7 The affected Vitest suites, `bun run lint`, and `bun run typecheck` pass without changing the gate default or rewriting historical `memory/episodic-log.md`.
<!-- AC:END -->
