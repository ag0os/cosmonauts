...
updatedAt: '2026-08-26T13:02:58.729Z'
---
...
<!-- AC:BEGIN -->
- [x] #1 Source-removed, source-unavailable, foreign-owner, incomplete, forget, and transfer-related check rows detect pending journals and concurrent manifest/journal changes and exit nonzero.
- [x] #2 The check path remains lock-free and does not create or mutate roots, locks, journals, manifests, targets, timestamps, files, links, or mtimes.
- [x] #3 A selected owner group uses one coherent manifest/journal before-and-after stability window rather than rereading the full manifest once per row.
- [x] #4 Integration tests mutate manifest and journal observations for catalogue-backed and stale/non-catalogue rows and fail if any outer check seam bypasses the stable observation contract.
