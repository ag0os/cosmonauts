...
updatedAt: '2026-09-02T13:59:47.129Z'
---
...
<!-- AC:BEGIN -->
- [x] #1 B-003 is green in `tests/extensions/agent-memory.test.ts` > `excludes retired knowledge until recall explicitly opts in`, with exact marker `@cosmo-behavior plan:living-memory#B-003`; combined-context injection and default recall expose only live records, while explicit opt-in returns retired records with original logical resource, physical retired path, and retired state.
- [x] #2 The retired subtree is excluded by default at the knowledge-store read boundary, opt-in paths are derived from contained originals rather than trusted persisted destinations, and user records are never mutation candidates.
- [x] #3 The extracted read semantics preserve existing live retrieval, combined-context bytes, profile exclusion, and explicit-save behavior, with no relocation-capable or source-removal code introduced.
- [x] #4 Project-native universal correctness evidence passes after every commit, and the Slice 0 B-001 receipt test and exact marker remain green at every commit.
<!-- AC:END -->
