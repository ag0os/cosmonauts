...
- **D-021 - Codex registers direct-directory links only; other link shapes are rejected** *(Added 2026-08-26 after the Checkpoint B sandbox probe)*
  - Decision: Link support is registered per target *and* per link shape, not per target alone. The Claude skill adapter registers `directory`, `flat-skill`, and `generated-wrapper` link shapes. The Codex skill adapter registers only `directory`. A `--link` selection that resolves to a flat-`SKILL.md` or generated-wrapper shape on Codex fails before any owner-root or manifest write, naming the asset, the resolved shape, and the shapes that target supports — the same pre-write rejection D-019 defines for command assets. Explicit link never falls back to copy, and no provenance is recorded for a rejected request. Copy mode is unaffected for every target and shape.
  - Evidence: a controlled Checkpoint B sandbox fixture varying only link shape (`/tmp/cosmonauts-harness-probe.L46mJG`) showed Codex 0.147.0 discovering a directory-symlinked skill but not either `SKILL.md` file-symlink shape; Claude Code discovered all three. Reproduced independently by the coordinator via `codex -a never exec --ephemeral --ignore-user-config --sandbox read-only`.
...
- Expected: never-managed assets copy with marker/provenance; managed assets retain recorded mode; explicit conversion reports old mode before rewriting; direct directory links and flat `<name>/SKILL.md` links have the registered local shape for targets that register that shape; generated wrappers link authored nodes and write generated nodes; repeated current sync writes nothing; `--link` against any command asset fails before owner-root/manifest writes with an actionable supported-mode error and leaves no provenance (D-019); `--link` resolving to a flat-skill or generated-wrapper shape on Codex fails the same way with a supported-shape error and leaves no provenance (D-021); remote/escaping/symlinked invalid shapes fail before owner-root/manifest writes
...
- **Link harness support.** *(Resolved 2026-08-26 at Checkpoint B.)* Probed in a controlled sandbox: Claude Code loads all three registered shapes; Codex 0.147.0 loads only a direct-directory symlink and ignores both `SKILL.md` file-symlink shapes. D-021 therefore registers link shapes per target and rejects unregistered shapes before any write. Explicit link still never falls back to copy. Slice C is unaffected because every live migration is copy mode against Claude.
