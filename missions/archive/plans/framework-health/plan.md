...
- **D-037 - The probe population includes `bundled/`; one census script replaces the audit suite**
  - Decision: D-032's production roots gain `bundled/`, which is shipped (package.json `files`); without it `tests/domains/coding-chains.test.ts`, which imports `bundled/coding/chains.ts`, fell out of the population. The census is `scripts/probe-census.ts` (`bun run probe:census`). It is deterministic for a commit, and it replaces the entire `scripts/test-health-audit/` suite. That suite served only the forfeit epoch method (D-002): its probe tool required a direct import route and an audit epoch (`review-8.md PR-001`). It is deleted with its tests and `docs/test-health-audit.md`. This also discharges D-036 (2) by deletion: `validateCalibrationRecord` no longer exists.
...
- **D-038 - Declarations are found by syntax; a mutation target may be shipped content**
  - Decision: (1) the census finds `it`/`test` declarations with the TypeScript AST, including multiline and chained forms such as `test.each([...])(...)`. A line regex missed one in `tests/packages/installer.test.ts`, so S57's median moved from line 309 to line 285. That declaration was probed afresh; the line-309 strengthening stays as an unsampled improvement. (2) A mutation target is shipped material a declaration actually exercises: code under `lib/`, `cli/`, `domains/`, `scripts/` or `bundled/`, or shipped content under `external-skills/` or `external-commands/` (package.json `files`). A declaration that pins the contract of an exact-pinned runtime dependency (`tests/pi-contract/`, Pi `@earendil-works/*` at the lockstep version) is probed in that dependency's shipped code, in a private copy of the package. It is never test code. S71 reads only shipped `external-skills/**/SKILL.md` and is probed there. S59 pins Pi's per-tool `sequential` dispatch and is probed in Pi's `agent-loop.js`; the cosmonauts code that depends on that behavior (`remember`'s `executionMode`) is protected by `tests/episodic/w3-contract.test.ts:450`. This corrects the coordinator-derived wording of D-036 (1) and TASK-711 AC #3; the human decision D-036 records, to carry review-8's findings into the tasks, is unchanged, and INV-005 is not weakened.
...
- **D-040 - The deleted spawn compiler stays deleted; the architecture record is amended**
  ...
  - Why: the QM (`missions/archive/reviews/qm/shared-rounds/review-round-3.md` F-002) found that the deletion contradicted D-012/D-014 as written, which is ratified ground. The human chose to amend the record over restoring the compiler as staged code (A) or unstaged (C).
