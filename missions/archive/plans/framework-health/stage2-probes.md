...
| S59 | `tests/pi-contract/pi-behavior-contract.test.ts`: one sequential tool serializes the entire batch | `node_modules/@earendil-works/pi-agent-core/dist/agent-loop.js:289-290` (Pi 0.80.6, private copy of the package; D-038) | per-tool `executionMode: "sequential"` ignored: only `config.toolExecution` serializes the batch | survived, then killed at its unit | the declaration builds its own tools and calls Pi's `runAgentLoop`; it pins Pi, not cosmonauts code. First probed at a cosmonauts dependent (`lib/extensions/agent-memory/index.ts:238`, `remember` executionMode set to "parallel"), where it survived; `tests/episodic/w3-contract.test.ts:450` kills that mutant. Re-targeted at the Pi code the declaration exercises: **killed** ("expected [ Array(4) ] to deeply equal [ 'tool_a:start', 'tool_a:end', …(2) ]"), green again after restore |
