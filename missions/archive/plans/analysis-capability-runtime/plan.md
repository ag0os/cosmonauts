...
- **D-029 - The delivered result contract cannot express gate coverage** *(amend-on-record against this archived plan; fix owned by `analysis-gate-coverage`)*
  - Decision: record that the shipped `changed-scope-audit` result is insufficient for its first consumer. A completed result carries one aggregate `verdict` plus `findings[]` whose `category` is a gate capability, but nothing expressing **which** gate categories were actually evaluated. A consumer therefore cannot distinguish "this gate was evaluated and clean" from "this gate was never evaluated", and `docs/analysis-capabilities.md` promises only "audit changes from an explicit base" — it never states coverage. The Fallow adapter happens to run dead-code, duplication, and complexity, but that is an adapter implementation detail, not a provider-neutral guarantee. The remedy is a coverage field on the completed result, validated against two real tools under INV-4, and is scoped as the separate plan `analysis-gate-coverage`. This entry does not itself change the contract.
  - Alternatives: let the consumer infer coverage from the absence of findings (rejected — an empty findings list is indistinguishable from an unevaluated category, so a gate the provider never ran would read as passed, exactly the silent pass INV-2 forbids); let the consumer read the provider's native payload to recover coverage (rejected — reintroduces provider coupling into a generic consumer, violating INV-1 and INV-4); relax the consumer to treat an aggregate `pass` as passing every bound gate (rejected — same silent-pass violation, and INV-3 requires an unclassifiable result to be reported as an error rather than guessed at).
  - Why: discovered by independent codex review of `analysis-gate-rewiring` (slice 2), which is the first consumer of this contract. Because that slice's Quality Manager correctly refuses to read native fields and correctly reports an unclassifiable gate as `failed-to-run`, every bound `duplication`/`complexity`/`dead-code` gate resolves to `failed-to-run` and sign-off can never be reached — including for this design's own plans. The gap is in the runtime contract this plan delivered, not in the consumer, so it is recorded here rather than worked around in a prompt, per that slice's standing rule that a contract gap is an amend-on-record against this plan and never a prompt-level workaround.
