...
This is a FOCUSED Stage 7 re-review (round 4).

Stage 7 reviews 2 and 3 found gaps in the host B-010 calibration: see `missions/plans/qm-chain-safety/stage7-review-{2,3}-{codex,claude}.md`. The coordinator's plan decision **D-031** (read it in `plan.md`) bounds the work. The host calibration is defense in depth over free-form model prose. It must meet two HARD FLOORS:
...
TASK-748 (`953d0f9`) remediated review 2. TASK-749 (`3ac2e24`) remediates review 3: read `missions/tasks/TASK-749*.md`. Review the diff from `f919594` to HEAD, in the context of the whole B-010 host calibration; ignore `missions/` except as documentation.

First, for every stage7-review-3 finding (codex 1–3, Claude H-1 and L-1..L-4), state whether it is RESOLVED, PARTIAL or OPEN, with file:line evidence. Check that its test would fail on the pre-TASK-749 code (`git show f919594:<path>`), and that the named mutations are killed. In particular, replace the Findings condition in the host ready check with `false`, and confirm that some test fails.
...
4. **Tests that cannot fail** (mutation-check the new ones), correctness, liveness, and scope creep beyond TASK-749. Also check the liveness side of the fail-safe rule: a normal clean report can still reach `ready`, and ordinary QM formatting does not make `ready` unreachable in practice.
