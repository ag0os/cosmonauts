...
updatedAt: '2026-07-30T19:28:49.014Z'
---
...
- **D-029 - The delivered result contract cannot express gate coverage** *(decided against `analysis-capability-runtime` and carried here, because this slice is the consumer that surfaced it; the fix is owned by `analysis-gate-coverage`)*
  - Decision: record that the shipped `changed-scope-audit` result is insufficient for this slice's Quality Manager. A completed result carries one aggregate `verdict` plus `findings[]` whose `category` is a gate capability, but nothing expressing which gate categories were actually evaluated, and the generic capability documentation promises only "audit changes from an explicit base". The remedy is a coverage member on the completed result, validated against two real tools under INV-4, scoped as the separate corrective plan `analysis-gate-coverage`. No prompt in this slice works around it.
  - Alternatives: infer coverage from the absence of findings (rejected — an empty findings list is indistinguishable from an unevaluated category, so a gate the provider never ran would read as passed, the silent pass INV-2 forbids); read the provider's native payload to recover coverage (rejected — provider coupling in a generic consumer, violating INV-1 and INV-4); relax this slice's Quality Manager to treat an aggregate `pass` as passing every bound gate (rejected — same silent pass, and INV-3 requires an unclassifiable result to be reported as an error rather than guessed at).
  - Why: independent codex review after this slice shipped proved the plan's own "a runtime gap may surface here" risk is real. This slice's Quality Manager is correct on both counts that create the deadlock — refusing native fields and reporting an unclassifiable gate as `failed-to-run` — so the contract is what must move, per this plan's standing amend-on-record rule.
...
- **REALIZED (2026-07-30) — the delivered contract cannot express gate coverage.** Independent codex review after this slice shipped proved the risk directly above is real. A completed `changed-scope-audit` result carries one aggregate verdict plus categorized findings, but nothing declaring which gate categories were evaluated, and the generic capability documentation never promises coverage. Because this slice's Quality Manager correctly refuses to read provider-native fields (INV-1) and correctly reports an unclassifiable gate as `failed-to-run` rather than guessing (INV-3), every bound `duplication`/`complexity`/`dead-code` gate resolves to `failed-to-run` and sign-off is unreachable — including for this plan's own ladder. Per the rule above the remedy is an amend-on-record against `analysis-capability-runtime` (recorded there as D-029), never a prompt workaround; it is owned by the corrective plan `analysis-gate-coverage`. Until that lands this slice's bound-gate resolution is inert, degrading safely rather than passing silently — the correct failure direction, but not a working gate.
