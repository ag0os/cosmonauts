...
| 3 | `mutation` | bindable | unbound | Negative tests kill unconditional scans/indexing, dropped source, missing payload, thrown warnings, mtime-derived terminal identity, duplicate/missing terminals, same-status capture, Drive-path per-task capture, and disabled metadata drift | pending | unbound; automated targeted tests plus reviewer judgment required |
