...
| S51 | `tests/memory/interface.test.ts`: exposes W1 taxonomy and honest write outcomes without speculative consolidation variants | `lib/memory/markdown-store.ts:456 failedWrite` | failure reason drops the underlying filesystem error message (': ${reason}' removed) | survived | strengthened, re-probe killed |
...
| S57 | `tests/packages/installer.test.ts`: rejects path dot when another domain is declared before creating the package store | `lib/packages/manifest.ts:176 hasNonExclusiveRootDomain` | off-by-one: domains.length > 2 instead of > 1 | survived | strengthened, re-probe killed |
...
First probe: 65 killed, 10 survived. After the survivor exits, every sampled declaration that still exists is recorded killed. Seven were strengthened and re-probed to killed, and one (S42) was deleted. S59 was killed once it was re-targeted at the unit it claims, and S11 was killed by the type-check step. No survivor closed on an edit alone.
