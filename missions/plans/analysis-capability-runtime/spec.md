# Spec — Analysis capability runtime

Slice 1 of 3 of the ratified `analysis-capabilities` spec (split by D-024,
2026-07-29). Grounded in the July 2026 Fallow investigation (`docs/fallow.md`,
`docs/fallow-workflow-integration.md`). This slice builds the capability
runtime — taxonomy, contract, provider adapter, process runner, Pi tools, and
agent-visible status. Consumer rewiring ships in `analysis-gate-rewiring`;
investigation procedures ship in `analysis-investigation-procedures`. CI
enforcement and the scheduled stewardship lane remain follow-up work.
...
This slice delivers the machinery that makes the above true at runtime. It
does not change which agents consume it: the legacy "Detected Analysis Tools"
prose injection is retained until the gate-rewiring slice deletes it, so no
shipped prompt is stranded against a missing block.
...
- **Quality Manager, Verifier, Fixer** — gain typed capability tools with
  structured findings (locations, severities, suggested actions) and honest
  bound/unbound resolution. This slice makes those tools exist and be correct;
  the gate-rewiring slice makes those roles consume them.
- **Planning agents (planner, plan-reviewer)** — gain read-only investigation
  capabilities (health, duplication, traces, project structure). This slice
  makes them callable; the investigation-procedures slice teaches the
  procedure.
- **Implementing agents (worker, refactorer)** — same relationship: tools
  first here, procedures later.
- **Project maintainers (human)** — swap or upgrade the backing analyzer for a
  language through project configuration, and see explicitly which
  capabilities are bound in a given project. Fully delivered by this slice.
- **Non-TS/JS projects adopting cosmonauts** — get whatever capabilities their
  ...
  pretending or breaking. Fully delivered by this slice.
...
supported scopes, declared metrics), unbound, failed, or detected-but-withheld
pending execution consent. No raw commands.

**Investigation.** An agent asks for complexity or duplication evidence on
given paths and receives structured findings. In a project whose provider
declares cyclomatic but not CRAP, a CRAP-specific question comes back as
unsupported-metric — not as zero findings. A scope kind the binding does not
advertise comes back as unsupported-scope — never silently widened.

**Failure.** A provider crash, invalid config, missing git base, cancellation,
or timeout surfaces as a tool error carrying capability, provider, failure
class, and process evidence. It is never a clean or empty result.

**Consent.** A detected provider is not executed until per-project execution
consent is recorded outside the repository. Until then, discovery stays
file-read-only and status names the detected provider and why execution is
withheld.

**Swap.** A maintainer replaces the backing provider in project configuration.
The next session shows the same capabilities bound to the new provider; tool
names are unchanged.
...
|---|---|---|
| AC-001 | this slice | Deliver in full |
| AC-002 | this slice | Deliver in full |
| AC-003 | this slice | Deliver in full |
| AC-004 | this slice | Deliver the runtime half (status, unbound tool returns, non-JS fixture); the gate-resolution clause lands in `analysis-gate-rewiring` |
| AC-005 | this slice | Deliver the tool-error half; the failed-to-run gate clause lands in `analysis-gate-rewiring` |
| AC-006 | this slice | Deliver in full |
| AC-007 | this slice | Deliver in full |
| AC-008 | this slice | Deliver in full |
| AC-009 | `analysis-gate-rewiring` | Not delivered here; the runtime must not foreclose it |
| AC-010 | `analysis-investigation-procedures` | Not delivered here; the runtime must not foreclose it |
| AC-011 | this slice | Deliver in full (pin, lossless normalization) |
...
- Runtime detection and binding, registered capability tools, and the
  agent-visible binding status.
- The signal-aware, non-mutating process runner and the execution-consent
  gate.
- Tests: contract tests for the binding and degradation behavior (including a
  no-provider fixture), the real-engine adapter tests, and the real-Pi error
  contract test.
- Provider and capability documentation, the public `lib/analysis/index.ts`
  entry, and the exact engine pin.

Excluded:

- Consumer rewiring of any kind — Quality Manager, Verifier, Fixer, planner,
  plan-reviewer, worker, refactorer prompts, agent tool allowlists, the shared
  analysis skill, and deletion of the concrete provider skill. Those are the
  two sibling slices.
- Deleting the legacy "Detected Analysis Tools" prose injection. It is
  retained by design until the gate-rewiring slice replaces its consumers.
...
- Registering the capability tools alongside the retained legacy injection is
  additive: no shipped prompt reads the new tools until the gate-rewiring
  slice, so this slice cannot regress current agent behavior.
