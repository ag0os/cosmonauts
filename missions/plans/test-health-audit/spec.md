...
- INV-006 - Heuristic conclusions remain human-reviewed until calibration establishes a reliable objective check. No heuristic finding becomes a CI failure merely because it can be automated.
- INV-007 - Feature development resumes only after confirmed weaknesses are remediated, critical behaviors and risks meet the ratified protection bar, residual uncertainty is explicit, and a human ratifies the baseline. Whole-project static health remains the later `project-health-audit`.

When speed, simplicity, or automation conflicts with evidence integrity, INV-001, INV-003, and INV-005 take precedence.
...
- Planners and reviewers, who need behavior- and risk-level protection evidence rather than test counts or marker presence.
- The project owner, who ratifies contract interpretations, residual uncertainty, gate promotion, and the return to feature development.
...
- `reasoned` - a human-reviewed causal argument without an executed probe;
...
A test expectation is never changed merely to agree with current production behavior. If contract sources conflict, the issue is `unresolved` until a human ratifies the intended behavior.
...
- [ ] AC-012 - Every confirmed test-health weakness is remediated or removed from claimed guardrail evidence without changing an expected product contract absent human ratification.
- [ ] AC-013 - The trustworthy baseline is established only when all critical behavior/risk portfolios are protected, suite integrity is demonstrated, required probes succeed, and remaining uncertainty is bounded and human-ratified.
- [ ] AC-014 - Objective checks and human-reviewed heuristics are distinguished explicitly; heuristic findings do not fail CI unless later calibration and human approval establish a reliable gate contract.
...
- Which contract conflicts or production violations discovered by corrected tests require separate human rulings before remediation can close?
