...
- **D-016 - Assessment work is bounded by identities and source span, not file count** *(bound reinterpreted as agent context by D-027, 2026-09-16)*
  - Decision: generate work units capped at 50 auditable profile identities, 2,500 relevant source lines, and eight source files, whichever is reached first. Oversized files split at top-level/nested suite boundaries, then declaration ranges if one suite remains oversized; each subdivision carries the same file-context/import/helper digest. Assessing-agent sessions write disjoint unit shards at the D-030 concurrency bound rather than at a human-attention limit. Actual unit count is census-derived; 18 is not assumed. At 3,154 current test declarations the 50-identity cap dominates the 8-file cap, so expect roughly 64-90 units.
  - Alternatives: D-005’s fifteen-file batches; one entire large file per session; one profile file per test.
  - Why: addresses `review-1.md PR-007`; current 7,205-, 3,017-, and 2,054-line test callbacks prove file count is not a workload bound. Under D-027 the cap sizes an assessing agent's context window and keeps one failed session cheap; it is no longer a human-attention budget. (The prior version of this rationale cited a "recorded memory-pressure incident" as arguing against wide concurrency. That incident is real — `missions/reviews/improvements/run-bf33d90b-0639-4b07-ad15-3e2f10194012.md` — but it records three background *watchers* being OOM-killed while "the Drive process itself was never affected", so it is evidence that cheap peripheral pollers die first, not that agent sessions cannot run in parallel. D-030 cites the single-process containment constraint instead.)
...
- **D-030 - One OS process per assessment unit, eight concurrent** *(Added 2026-09-16 on user ruling)*
  - Decision: each assessment work unit runs as its own OS process through a driver process backend (`lib/driver/backends/cli-process.ts` or `cosmonauts-subagent.ts`), never as an in-process `spawn_agent` child. Default concurrency is **8**, passed as the `resolveMaxConcurrentSpawns` override (`lib/orchestration/spawn-limits.ts` ships a default of 5, which this deliberately overrides). Each unit records its wall-clock duration and peak RSS in the epoch so the next change to this number is measured rather than re-argued.
  - Alternatives: the shipped in-process default of 5; in-process execution at 8; an unpinned "machine-bounded" bound, which is what this decision replaces.
  - Why: the bound and the execution path are one decision, not two. `lib/orchestration/agent-spawner.ts:103` creates "an ephemeral in-memory session", and `missions/architecture/orchestration-future.md:257-260` records that on that path "a child that OOMs or hits a fatal takes the whole process and every sibling down" — so in-process concurrency compounds risk: more heap raises OOM probability *and* each OOM costs more siblings. One process per unit makes containment per-unit, so a fatal costs exactly one resumable unit and 8 is comfortable at ~200-500 MB per process against 36 GB. Units are already disjoint, atomically published, and resumable, so concurrency has no correctness role — it only trades throughput against re-work, which is why it is safe to tune on measurement.
...
- **D-037 - A `repair-required` disposition is published, not absorbed** *(Added 2026-09-17 on eighth-live-run evidence)*
  - Decision: `reconcileCensus` publishes a structured `repairRequired` surface on `CensusResult`, restates each row as residual uncertainty, and `suite-integrity.md` renders them in their own section with assessor and reasoning. `TASK-701` AC #8 consumes those rows as remediation-ledger input. Census `clean` keeps its meaning — every finding has an *answer* — because remediation is stage 8's job; what changes is that "needs repair" is no longer indistinguishable from "resolved".
  - Alternatives: making a repair-required row hold the census not-clean, which would deadlock stages 2-5 behind stage-8 remediation that has not run and conflicts with the plan's own split between census integrity and remediation; leaving the gap and recording it in the packet.
  - Why: run 8's C2 caught a real flake in the act — `tests/plans/archive.test.ts` failed a full-suite correctness run on an untracked episode lock and ENOTEMPTY cleanup while both isolation runs and an immediate isolated rerun passed all 20 cases — and the assessor classified it `repair-required-suite`, stating it belonged in "the stage-8 remediation ledger and explicit residual uncertainty". Neither existed. `applyDispositions` set `accountedFor: disposition !== undefined`, so *any* answer made the finding accounted-for and the census `clean: true`; the strings `repair-required-tooling`/`repair-required-suite` appeared only in a validation list and a type union, with **no consumer anywhere**, and zero occurrences in `suite-integrity.md`. The row survived solely as one entry among 19,005 in a 22 MB `dispositions.json`. That is the state-with-no-exit class that aborted run 2 and that two review channels caught twice (D-020, D-021), reappearing one level up: the disposition *values* had a home, but `repair-required` had no exit. It was visible only because an honest assessor used the value — had it said `accounted-for`, nothing observable would differ.
  - Decided by: user ratified, 2026-09-17.

- **D-038 - Something must own running the units, not just describing how they run** *(Added 2026-09-17 on ninth-live-run evidence)*
  - Decision: stage 6 ships a resumable dispatch command that issues every pending work unit to an assessing-agent session as one OS process per unit through a driver process backend at the D-030 bound, recomputing pending work from published shards. `TASK-696` AC #12 owns it. Preparation, publication, and validation primitives do not constitute execution.
  - Alternatives: letting the P-final checkpoint orchestrate units with a run-local script, which run 9's worker attempted and correctly abandoned because AC #5 requires the driver mechanism, not an ad-hoc process; treating "assessing agents" as an ambient capability, which is the assumption that failed.
  - Why: run 9 reached stage 6 with a valid 70-unit queue covering all 3,120 identities and published zero profiles. `B-004` specifies the shard contract ("assessing agents complete disjoint NDJSON unit shards and the epoch validator aggregates them") and `D-030` specifies the execution model (one OS process per unit, eight concurrent, driver backend), but no design section, task, or CLI command owned turning queued units into running agents — the audit interface shipped `census | prepare-units | validate | probe | baseline`, which can prepare and verify shards but cannot produce them. Every AC described what a published unit must *look like*; none required a component that *runs* one. Stages 7-11 were checked for the same shape of gap and have none: `probe`, `validate`, and `baseline` cover them.
...
Dispositions live in `<epoch>/dispositions.json`, written by an assessing agent at C2 and read by the collector when it computes census state. Each entry names the finding it answers, the disposition (`accounted-for`, `limitation-accepted`, `repair-required-tooling`, `repair-required-suite`), the agent's reasoning, the **sources it actually opened** to reach that reasoning, and its assessor provenance. The two repair dispositions are separated because they belong to different stages: `repair-required-tooling` is a defect in the audit's own collector and is fixed before the census checkpoint passes, while `repair-required-suite` is a real finding about the audited suite — a flaky test, an unestablishable error phase — which becomes a remediation-ledger row at stage 8 and carries as explicit residual uncertainty until then. A census checkpoint blocks on the former and never on the latter; otherwise a genuine suite defect would halt the audit at stage 3, which is the stage that discovers it. This is what gives `incomplete` and `blocked` a defined exit: the plan has always required that every mismatch, skip, filter, and limitation carry a disposition, but before this amendment it never said where dispositions live or who writes them, so the first live run had no way to leave `blocked`.

Census state is `complete`, `incomplete`, or `blocked`. `incomplete` exits only after every mismatch/skip/filter/limitation has a recorded disposition and any recollection that disposition calls for; `blocked` exits only after execution/collection/assessment repair, or after a `limitation-accepted` disposition records why the limitation is irreducible — a `test.each(<variable>)` parameter set that is genuinely not statically countable is the live instance. A finding without a disposition never becomes clean, which is `INV-005` unchanged: dispositioning makes evidence accounted-for, not invisible.

`limitation-accepted` is the disposition most able to launder a defect, so it carries an extra burden: it is valid only when the agent opened the cited source and its reasoning states what it found there. Restating the collector's own message back as a justification is not assessment. The live instance: the third run's assessing agent accepted all nine `unsupported-syntax` findings as irreducible because "the declaration uses a variable parameter set", when eight of the nine were literal arrays wrapped in `satisfies` or `as const` — statically countable, and a collector defect rather than a limitation. It never opened the files. This is Risk 5 in the flesh, and it is why `consultedAuthorities` is a required field rather than a courtesy. Neither can render clean. Raw full-suite collection before calibration is inventory data only; no health/clean conclusion is trusted until calibration passes.
...
`audit/index.json` atomically names `currentEpochId` and prior epoch IDs. Each `audit/epochs/<id>/manifest.json` is immutable and contains evaluated HEAD, complete working-tree content digest for material inputs, command definitions, method/schema version, census/inventory digests, authority/SUT input digests, and deterministic work-unit assignments. Each published unit carries its wall-clock duration and peak RSS so the D-030 concurrency bound is tuned on recorded cost. No correctness outcome depends on an in-memory progress map.

Work units are lexically deterministic after enforcing D-016 caps. Large files split at suite/declaration boundaries and each unit receives shared file-context/import/helper evidence. One assessing agent owns one unit and unit paths are disjoint. Each unit runs as its own OS process at the D-030 bound of eight concurrent, and records its wall-clock duration and peak RSS in the epoch alongside its shard. Each unit writes a temporary sibling and atomically renames after validation. Progress is reconstructed from valid current-epoch unit files. A failed session loses at most its bounded unit.
...
9. Stage-6 units run one OS process per unit at the D-030 bound, and every published unit carries its wall-clock duration and peak RSS.
10. The audit reaches `eligible-for-ratification` with no human input at any stage, and `baseline.md` carries exactly one `## Ratification packet` section holding every question the owner must answer.
11. Establishment requires all eight rows plus owner ratification of an already-committed evaluated revision/canonical digest; any material change makes it stale.
...
6. **Profile units — B-004.** Generate D-016 work units, run disjoint assessing-agent sessions as one OS process per unit at the D-030 bound of eight concurrent, validate/atomically publish each, and resume from persisted current-epoch units. Re-sample controls between waves. **P-final:** identity set equality, workload caps, complete agent-assessment provenance, no stale unit.
