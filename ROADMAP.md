...
Per-project `fallow` execution consent was granted 2026-09-10 while preparing `chain-stage-context`, which bound `duplication`, `complexity` and `dead-code`. Three committed files — `.fallow-baselines/dead-code.json`, `health.json`, and `dupes.json` — serve as changed-scope regression floors. They were adopted as-is at their last-writer commits (dead-code `eb55243`; health and dupes `6b36c80`), then re-anchored once at local `main` `29fc0ce` under human ruling N-001 (plan D-029). The coordinator found that the adopted floors missed 3 dead-code, 15 duplication, and 217 health findings already on `main`; the re-anchoring keeps that inherited debt from failing changed-scope audits. The manifest records each digest and the reason. The current committed floors contain 27 unused exports, 103 unused types, 1 unused class member, 2 duplicate-export pairs, 295 health finding counts, and 92 clone groups. The earlier 2026-09-10 scan's 3,288 duplicated lines (3.9% across 50 files) were informational and are historical figures, not a current census.

Baseline refresh is explicit: `bun run refresh:fallow-baselines -- --base <revision> --reason '<reason>' --category <dead-code|health|dupes>`. It analyzes the resolved base in a private temporary checkout, writes selected floors to the working repository, and appends provenance; ahead or dirty working-tree findings cannot enter the floors. Ordinary review cannot write a baseline. New source suppressions require a human-listed entry in the base revision of `.cosmonauts/suppression-exceptions.json`; `bun run check:suppressions -- --base <revision>` enforces that ownership. Same-change registry edits do not grant an exception.

**Part 1 — retire the dead-code baseline.** The current floor records 27 unused exports, 103 unused *type* exports, 1 unused class member, and 2 duplicate-export pairs. The 2026-09-10 `fallow dead-code` scan exited 1 with 26 unused exports, 102 unused *type* exports, and 2 duplicate-export pairs. Independently pickable, no sequencing constraint.

- In the 2026-09-10 dry run, `fallow fix` auto-handled 25 of 26 value exports — it stripped the `export` keyword and left the symbol, so internally-used constants like `CODEX_ARGS_ENV` were safe
- The current floor's **103 unused type exports are not auto-fixable** and are the bulk of the work. Decide per cluster: delete, or suppress with a stated reason. The 2026-09-10 scan counted 17 in `lib/harness-adapters/sync.ts` and 14 in `scripts/validate-harness-exports.ts`
...
**Part 2 — complexity pass. Sequenced *after* `chain-stage-context` ships, deliberately.** The current health floor records 295 finding counts. The 2026-09-10 `fallow health` scan exited 1: 74 functions above threshold across 12,574 analysed, maintainability 90.9 ("good").
...
- Evidence: `missions/plans/chain-stage-context/plan.md` Design §7a (the 2026-09-10 measurements and audit invocation) · `.fallow-baselines/manifest.json` (current floors and provenance) · consent record at `~/.cosmonauts/analysis-execution-consent.json`, user-owned by design — the repo cannot grant itself execution permission
