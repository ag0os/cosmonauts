...
- Add polyglot provider routing and validate a second executable provider; until then a non-JavaScript/TypeScript project binds no provider and surfaces all seven capabilities explicitly as unbound. Targets the user's project (any codebase), not just cosmonauts — per-language analyzers (ESLint, ruff/mypy, clippy, …) and how a domain surfaces the right one per project.
