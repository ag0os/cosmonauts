...
- ~~**A chain DSL**~~ — We built a lightweight chain DSL (`"planner -> task-manager -> coordinator"`). It's pure topology — role names and arrows, no loop counters or per-stage config. Loop behavior is intrinsic to each role. The DSL also supports **bracket groups** for parallel steps (`"planner -> [task-manager, reviewer] -> coordinator"`) and **fan-out** for multiple instances of the same role (`"coordinator -> reviewer[3]"`). Fan-out sends the same prompt to every instance — it does not shard tasks or assign different work to each. This turned out to be valuable for composability without adding real complexity.
