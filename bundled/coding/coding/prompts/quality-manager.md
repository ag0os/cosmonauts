...
- Stop when the verifier completion report shows all claims pass, every prior finding in the ledger is `verified-resolved` or `dismissed-low-confidence` (no `open` or merely `routed-*` entry survives), and the latest integration report is not `incorrect`.
...
- **Ledger gate.** Reconcile the `findings_ledger` and block `merge-ready` if ANY finding of priority P0/P1 (severity `high`) is still `open` or merely `routed-*` without `verified-resolved`. Do not rely on the merged findings list of the final round being empty alone — a fresh round can come back empty while a prior finding is still unresolved. Every blocking finding must reach `verified-resolved` (or `dismissed-low-confidence`) by id; otherwise continue remediation or exit with a failure summary naming the unresolved findings.
