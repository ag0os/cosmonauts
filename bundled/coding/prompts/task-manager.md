...
When one behavior applies the same rule across multiple named seams, prefer one assertion per seam in the task ACs. If keeping the behavior clustered is the better task boundary, write the AC so it explicitly enumerates every seam the worker must implement and test; a single broad "rule is enforced" AC is not enough because it can pass with only partial seam coverage.
