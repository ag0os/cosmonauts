...
6. **Sweep Design, Decision Log, and Files to Change for constraints the behaviors don't carry.** Behaviors are covered by construction in step 5 — constraints that live only in prose are the ones that evaporate. Walk every Decision Log entry, every load-bearing design rule, and every file in the plan's Files to Change list: each must land in some task's ACs or description. A checkpoint or final-verification task does **not** count as the owner — it detects the gap only after the implementing tasks are closed, forcing rework across them; assign the constraint to the task that touches that seam. If a constraint has no natural owning task, report the plan-coverage gap instead of silently dropping it.
