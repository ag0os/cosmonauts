...
- **Verifier-native failures**: failed project-native checks and failed verifier contract criteria (`QC-*` with `verification: verifier`) route to `fixer` for immediate remediation. These are high-priority regardless of assessed severity. Do not create remediation tasks for verifier-native failures. When the failed claim is a codebase audit from a detected analysis tool, include the full audit output from the verifier evidence in the fixer prompt — the structured findings tell fixer exactly what to remove or restructure. For these auxiliary analysis-tool findings (complexity, duplication, audit), the fixer prompt MUST carry an explicit minimal-change constraint: make the NARROWEST change that clears the specific flagged finding at the flagged location — do NOT refactor passing code to satisfy a metric, and do NOT enlarge the diff beyond what the finding requires. Only act on auxiliary-tool findings that block a binding gate.
...
Abstract `gate_ladder_rows` do not create remediation by themselves. They affect remediation only through mapped verifier/reviewer evidence that already failed, legacy `QC-*` criteria, detected project-native analysis tools, or integration/reviewer findings. Keep `degraded_gates` and `protocol_pending_gates` as explicit reporting items unless separate executable evidence exists.
...
- **Reconcile the ledger.** Before treating any finding as resolved, reconcile each `routed-*` entry against evidence. A routed finding becomes `verified-resolved` ONLY via one of: (a) the `fixer` reported it resolved by id AND the change is visible in the diff, or (b) the reviewer's finding-anchored `## Prior Findings` section marked it `resolved` with evidence. A fresh re-review that simply returns "no findings" does NOT close a prior finding. If the re-review marks a prior finding `unresolved`, it stays `open` (re-route it in step 5); never let it silently disappear.
- Stop when the verifier completion report shows all claims pass, every ledger entry has reached a terminal disposition (`verified-resolved` / `dismissed-low-confidence` / `deferred`), and the latest integration report is not `incorrect`. A finding may only become `deferred` under the round-budget rule in step 7 — within the remediation loop, keep re-routing `open`/`routed-*` findings rather than abandoning them. If the round budget is exhausted with P0/P1 findings still non-terminal, do not force a stop: exit with a failure summary instead (they block sign-off in step 7).
