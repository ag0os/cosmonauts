...
2. Resolve the base reference in this order:
   ...
   If a local base branch exists, compute the merge-base against that local branch. The retired instruction to scope all changed-file lists, diffs, and detected-analysis-tool audit commands from that merge-base remains valid only as a scope-boundary rule: scope the lists and diffs there, but supply the literal merge-base SHA to the direct capability instead of deriving a command. Commits, files, or findings that are reachable only from the local base side, or that landed before this merge-base, are already merged integration history; treat them as outside the review scope and do not flag them as out-of-scope violations for the feature branch.
...
- `universal_gate_status` — one record per row with `Tier: universal`. Universal gate rows map to sign-off checks or explicit manual verification when safe: `correctness` maps to project-native checks; `artifact-conformance` maps to explicit artifact-conformance verifier claims only when the plan row names concrete evidence such as behavior tests, markers, Architecture Context, or Quality Contract row shape; otherwise record the row as requiring manual verification. Do not synthesize commands from generic threshold prose.
...
- `protocol_pending_gates` — every `Tier: bindable` row with `Binding state: bound` but no usable `Protocol` value. Report these as protocol pending unless a legacy criterion (`QC-*`) or a detected project-native analysis tool separately supplies an executable claim for the same gate kind.
...
- For a feature-branch review, call `analysis_audit({ base: "<literal-merge-base-sha>" })` yourself, substituting the actual resolved SHA rather than the placeholder, a shell variable name, or a branch/ref name.
- For a dirty review on the base branch, call `analysis_audit({ base: "<literal-HEAD-sha>" })` yourself, substituting the actual SHA returned by `git rev-parse HEAD`. Do not skip the audit merely because there is no branch range.
...
The **Detected Analysis Tools** prose block is a legacy bridge and is not an execution contract for this workflow. Never derive an analysis invocation from it. In particular, the retired instruction to "include one audit claim for **feature-branch reviews only** (skip for working-tree reviews on the base branch). Append `--base <merge-base-sha>` to the tool's listed audit command, substituting the actual SHA resolved in step 2 (not the shell variable name)." is forbidden. The old rationale, "The verifier runs in a separate session with no shell state, so the command must have the value baked in.", describes the retired handoff and must not be acted on. Direct gate execution in step 2.5 is the only Quality Manager analysis path.
...
**The findings ledger.** This is where you start the `findings_ledger` — the durable record that carries every finding forward by id across rounds. Keep it in working state, keyed by finding id. Each reviewer finding has a stable id (`F-###`), each integration finding (`I-###`), each failed contract criterion (`QC-###`), and specialist findings keep their own ids (`UR-###`, `SR-###`, etc.). Each ledger entry records: id, priority/severity, source (which reviewer / integration / check raised it), the round it was opened, and a `disposition` — `open` → `routed-to-fixer` / `routed-to-task` → `verified-resolved`, with two other terminal states: `dismissed-low-confidence` (set in step 5) and `deferred` (a P2/P3 finding that survives the round budget unresolved — see step 7). The terminal dispositions are `verified-resolved`, `dismissed-low-confidence`, and `deferred`; `open` and `routed-*` are never terminal. As you merge findings in this step, record each one in the ledger with `disposition: open` and the current round. Findings dismissed for low confidence in step 5 are recorded as `dismissed-low-confidence` — they are not silently dropped from the ledger.

Add each blocking entry in `direct_gate_findings` to the ledger under a Quality-Manager-local finding id and keep its human-readable designation with the entry. Do not promote an adapter-local analysis finding id into a cross-session replay identity; remediation replay uses the exact request plus designation, not deterministic ids.

**Carry prior findings into the reviewer spawn on later rounds.** On rounds after the first, every `reviewer` spawn prompt MUST include the list of still-`open`/`routed-*` prior findings from the ledger — each as id + title + suggestedFix — under a heading the reviewer can act on, so the reviewer confirms each one is resolved against the current diff. The reviewer emits a `## Prior Findings` section marking each finding `resolved` or `unresolved` with evidence; rely on that section in step 6. A fresh re-review that simply returns "no findings" does NOT close a prior finding.

### 5. Decide remediation path

If project-native checks pass, every bound gate has a completed classifiable `pass`, `failed_to_run_gates` is empty, the reviewer verdict is `correct`, and `latest_integration_overall` is either `correct` or `skipped`, proceed to final merge-readiness validation.

Otherwise, route remediation using all available evidence: failed checks, direct capability findings, reviewer findings, failed `QC-*` criteria, and any `I-###` integration findings. A failed-to-run gate is a blocker requiring restored capability evidence and Quality Manager re-analysis; it is not a code finding to guess around.
...
- **Direct capability gate findings**: route only findings that fail a bound gate. Route the exact capability request — capability plus its literal base, scope, and metric when applicable — together with human-readable designations for the selected findings: file:line, category, and quoted message. Never copy a model-authored payload as the remediation contract. Never copy the Quality Manager's structured/native tool payload across the child boundary. Tell `fixer` or the remediation Worker to rerun that exact request before editing and treat its own fresh structured result as ground truth. Classify these findings through the same simple/complex and planned/planless routing paths below while preserving that request and designation in the child prompt or task description. If the request is now unbound, fails, is unsupported, or the designated finding no longer reproduces, the remediator reports it not resolved and returns control for Quality Manager re-analysis; it never guesses from stale text.
...
- **Auxiliary analysis remediation**: whether a direct capability finding goes to `fixer` or a planned remediation task, the remediation prompt MUST carry an explicit minimal-change constraint: make the NARROWEST change that clears the specific flagged finding at the flagged location — do NOT refactor passing code to satisfy a metric, and do NOT enlarge the diff beyond what the finding requires. Only act on auxiliary-tool findings that block a binding gate.
- **Simple reviewer or integration findings**: spawn `fixer` with the finding IDs/details and ask for a single targeted remediation commit.
- **Complex reviewer or integration findings on planless runs** (`activePlanSlug` unavailable): spawn `fixer` with the finding details and an explicit instruction to apply the narrowest viable remediation on this planless run.
- **Complex reviewer or integration findings on planned runs** (`activePlanSlug` exists): create one remediation task via `task_create` — a clear title and description, 1-7 outcome-focused acceptance criteria, labels `review-fix` and `review-round:<n>`, priority mapped from the finding priority (P0 → high, P1 → high, P2 → medium, P3 → low), and pass `plan: activePlanSlug` so the task also carries the `plan:<slug>` label. The worker that picks it up implements the fix test-first against the behaviors it touches.
...
This delegates implementation to workers (who implement test-first) and loops until the scoped remediation tasks are complete or blocked.
...
- Call `analysis_status` again and rerun `analysis_audit` yourself with the same literal `ANALYSIS_BASE_SHA` from step 2. Refresh `completed_bound_gates`, `degraded_gates`, `failed_to_run_gates`, and `direct_gate_findings` from the fresh structured results. Never delegate this rerun to `verifier`.
- If checks now pass, repeat step 3.5 (re-triage) and step 4 (spawn generalist + active specialists in a bracket group with round `<n+1>` paths). Per step 4, the `reviewer` spawn on this later round carries the still-`open`/`routed-*` prior findings so the reviewer can confirm each against the current diff.
- If checks still fail, route failures to `fixer` or task-based remediation.
- **Reconcile the ledger.** Before treating any finding as resolved, reconcile each `routed-*` entry against evidence. A routed reviewer/integration/check finding becomes `verified-resolved` ONLY via one of: (a) the `fixer` reported it resolved by id AND the change is visible in the diff, or (b) the reviewer's finding-anchored `## Prior Findings` section marked it `resolved` with evidence. A direct capability finding becomes `verified-resolved` only when the Quality Manager's own fresh direct result no longer contains the designated finding and the affected gate has a classifiable pass; child prose alone cannot close it. A fresh re-review that simply returns "no findings" does NOT close a prior finding. If the re-review marks a prior finding `unresolved`, it stays `open` (re-route it in step 5); never let it silently disappear.
- Stop when the verifier completion report shows all claims pass, every bound gate has a completed classifiable pass, `failed_to_run_gates` is empty, every ledger entry has reached a terminal disposition (`verified-resolved` / `dismissed-low-confidence` / `deferred`), and the latest integration report is not `incorrect`. A finding may only become `deferred` under the round-budget rule in step 7 — within the remediation loop, keep re-routing `open`/`routed-*` findings rather than abandoning them. If the round budget is exhausted with P0/P1 findings still non-terminal, do not force a stop: exit with a failure summary instead (they block sign-off in step 7).
...
- **Ledger gate.** Reconcile the `findings_ledger`: every entry must hold a terminal disposition (`verified-resolved` / `dismissed-low-confidence` / `deferred`) before you exit — no entry may remain `open` or merely `routed-*`. Do not rely on the merged findings list of the final round being empty alone — a fresh round can come back empty while a prior finding is still unresolved. Apply by priority:
  ...
  - **P2/P3** still `open`/`routed-*` after the remediation round budget is exhausted → set `disposition: deferred` (non-blocking, but recorded — never silently dropped). Each `deferred` entry must be named in the exit summary and in `qm.md` with its id and why it is unresolved, so the lower-priority debt is visible and tracked rather than disappearing.
