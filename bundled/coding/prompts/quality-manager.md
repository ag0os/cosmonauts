...
On every invocation that has a review scope — with or without an active plan — resolve every gate kind that has a runtime capability: `duplication`, `complexity`, `dead-code`, and `boundary-conformance`. Call `analysis_status` yourself first. Do not resolve only the gates the changed-scope audit happens to cover: a gate kind you never resolve can hide a failed binding. Runtime status is the only authority on whether a gate is enforceable.
...
Consume the complete structured tool result directly, including its per-gate verdict evidence and gate-aligned findings. Require the result to echo `capability: "changed-scope-audit"` and the exact changed scope/base requested. Resolve `duplication`, `complexity`, and `dead-code` only from that completed audit result, and `boundary-conformance` only from its own capability's completed result — even when the audit also declares boundary coverage, so the gate is resolved once. The same rules apply to whichever result resolves the gate:
...
- Call `analysis_status` again, rerun `analysis_audit` yourself with the same literal `ANALYSIS_BASE_SHA` from step 2, and rerun the boundary capability when `boundary-conformance` is bound. Refresh `completed_bound_gates`, `degraded_gates`, `failed_to_run_gates`, and `direct_gate_findings` from the fresh structured results. Never delegate this rerun to `verifier`.
