---
name: fallow
description: Codebase intelligence CLI for JavaScript and TypeScript. Finds unused code (files, exports, types, deps), code duplication, circular deps, complexity hotspots, architecture boundary violations, and feature flag patterns. Use when asked to audit code health, find dead code, detect duplicates, check complexity, check architecture boundaries, clean up a codebase, auto-fix unused exports/deps, set up dead-code CI gates, or run fallow. Do NOT load for type checking (use tsc), lint/style (use ESLint/Biome/Prettier), security scanning, bundle size, runtime debugging, or non-JS/TS projects.
license: MIT
metadata:
  author: Bart Waardenburg
  upstream: https://docs.fallow.tools
---
...
- `/skill:typescript` — TypeScript patterns and conventions when interpreting unused-export findings
- `/skill:refactoring` — Discipline for safely removing dead code and consolidating duplicates
- `/skill:engineering-principles` — Design guidance when deciding whether flagged code should be deleted, kept, or restructured
